...
def handle_download():
    ...
    final_filename = f"{timestamp}_{initial_title}_{quality}.mp4"
    final_path = Path(f"{downloads_dir}/{final_filename}")

    command = [
        'yt-dlp',
        '--cookies', 'cookies.txt',  # If you have a cookies file
    ...
    def update_progress():
        nonlocal initial_title, quality, duration_minutes, final_filename, final_path
        json_data = []
        ...
        for line in process.stdout:
            try:
                ...
                if 'total_bytes_estimate' in json_line and 'downloaded_bytes' in json_line:
                ...
                if 'title' in json_line:
                    ...
                    jobs[job_id]['title'] = initial_title  # Store title, but don't rename yet
    ...
    if process.returncode == 0:
        ...
        final_title = jobs[job_id]['title']  # Retrieve the title
        ...
        if final_path.exists() and new_filename != final_filename:
            os.rename(final_path, new_path)
            final_filename = new_filename
            final_path = new_path

        jobs[job_id]['title'] = final_title
    ...
    return jsonify({'jobId': job_id, 'filename': jobs[job_id]['filename']})
